			...
			<div class='col-sm-12'>
                <div id="box" class='mt-4' style='height: 20rem;width:100%;overflow-y: scroll;overflow-x: hidden;position: relative;'>
                    ...
                    <?php
                        ...
                        if (!isset($_POST['pedir']) == FALSE) {
                            // campos e variáveis
                            $id_proj = $_GET['proj'];
                            $descricao = $_POST['descricao'];

                            // inserir pedido
                            $insert_pedi = "INSERT INTO pedidos (descricao, criador, proj) VALUES (:descricao, :criador,  :proj);";
                            $stmt = $conn -> prepare($insert_pedi);
                            ...
                            if ($run) {
                                echo "<script>alert('Seu pedido foi registrado!');</script>";
                                echo "<script>window.location.href='projeto.php?cod=$id_proj'</script>";
                            } else {
                                echo "<script>alert('Ocorreu um erro ao pedir para participar!');</script>";
                            }
                        }
                    ...
                    <form method= "POST">
                        ...
                        <label for='desc'>Descreva seu pedido</label><br>
                        <textarea class='rounded col-sm-6' style='border: 1px solid white;padding:8px;' type='text' name='descricao' id='desc' required></textarea><br><br>

                        <input class='btn btn-light' type='submit' name='pedir' value='Pedir'><br><br>

                        <b>* Não passe informações pessoais (bancárias, por exemplo)</b>

                    </form>

                    <hr>
                    
                    <a href='projeto.php?cod=<?php echo $id_proj?>'><button class='btn btn-success mr-3'>Voltar</button></a>
                </div>
            </div>
        </div>
    </div>
